...
#define MPL_LOGV(fmt, ...)                                                                                             \
    do                                                                                                                 \
    {                                                                                                                  \
        __pragma(warning(suppress : 4127)) if (0) MPL_LOG(LOG_VERBOSE, MPL_LOG_TAG, fmt, ##__VA_ARGS__);               \
...
#define MPL_LOGV_IF(cond, fmt, ...)                                                                                    \
    ...
    {                                                                                                                  \
        if (0)                                                                                                         \
            MPL_LOG(fmt, ##__VA_ARGS__);                                                                               \
    } while (0)
#else
#define MPL_LOGV_IF(cond, fmt, ...)                                                                                    \
    ((CONDITION(cond)) ? MPL_LOG(LOG_VERBOSE, MPL_LOG_TAG, fmt, ##__VA_ARGS__) : (void)0)
#endif
#endif
...
#ifndef MPL_LOGD_IF
#define MPL_LOGD_IF(cond, fmt, ...) ((CONDITION(cond)) ? MPL_LOG(LOG_DEBUG, MPL_LOG_TAG, fmt, ##__VA_ARGS__) : (void)0)
#endif
...
#ifndef MPL_LOGI_IF
#define MPL_LOGI_IF(cond, fmt, ...) ((CONDITION(cond)) ? MPL_LOG(LOG_INFO, MPL_LOG_TAG, fmt, ##__VA_ARGS__) : (void)0)
#endif
...
#ifndef MPL_LOGW_IF
#define MPL_LOGW_IF(cond, fmt, ...) ((CONDITION(cond)) ? MPL_LOG(LOG_WARN, MPL_LOG_TAG, fmt, ##__VA_ARGS__) : (void)0)
#endif
...
#ifndef MPL_LOGE_IF
#define MPL_LOGE_IF(cond, fmt, ...) ((CONDITION(cond)) ? MPL_LOG(LOG_ERROR, MPL_LOG_TAG, fmt, ##__VA_ARGS__) : (void)0)
#endif
...
#define MPL_LOG_ALWAYS_FATAL_IF(cond, fmt, ...)                                                                        \
    ((CONDITION(cond)) ? ((void)android_printAssert(#cond, MPL_LOG_TAG, fmt, ##__VA_ARGS__)) : (void)0)

#define MPL_LOG_ALWAYS_FATAL(fmt, ...) (((void)android_printAssert(NULL, MPL_LOG_TAG, fmt, ##__VA_ARGS__)))
...
#else
#define MPL_LOG_FATAL_IF(cond, fmt, ...) MPL_LOG_ALWAYS_FATAL_IF(cond, fmt, ##__VA_ARGS__)
#define MPL_LOG_FATAL(fmt, ...) MPL_LOG_ALWAYS_FATAL(fmt, ##__VA_ARGS__)
#endif
 ...
 */
#define MPL_LOG_ASSERT(cond, fmt, ...) MPL_LOG_FATAL_IF(!(cond), fmt, ##__VA_ARGS__)
...
#else
#define MPL_LOG(priority, tag, fmt, ...) MPL_LOG_PRI(priority, tag, fmt, ##__VA_ARGS__)
#endif
#endif
...
#ifndef MPL_LOG_PRI
#ifdef ANDROID
#define MPL_LOG_PRI(priority, tag, fmt, ...) LOG(priority, tag, fmt, ##__VA_ARGS__)
#elif defined __KERNEL__
#define MPL_LOG_PRI(priority, tag, fmt, ...) pr_debug(MPL_##priority tag fmt, ##__VA_ARGS__)
#else
#define MPL_LOG_PRI(priority, tag, fmt, ...) _MLPrintLog(MPL_##priority, tag, fmt, ##__VA_ARGS__)
#endif
#endif
...
#ifdef ANDROID
#define MPL_LOG_PRI_VA(priority, tag, fmt, args) android_vprintLog(priority, NULL, tag, fmt, args)
#elif defined __KERNEL__
/* not allowed in the Kernel because there is no dev_dbg that takes a va_list */
#else
#define MPL_LOG_PRI_VA(priority, tag, fmt, args) _MLPrintVaLog(priority, NULL, tag, fmt, args)
#endif
#endif
...
#define LOG_RESULT_LOCATION(condition)                                                                                 \
    do                                                                                                                 \
    {                                                                                                                  \
        __print_result_location((int)(condition), __FILE__, __func__, __LINE__);                                       \
        __pragma(warning(suppress : 4127))                                                                             \
    } while (0)
#else
#define LOG_RESULT_LOCATION(condition)                                                                                 \
